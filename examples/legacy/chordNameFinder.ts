import { analyzeIntervals, hasInterval, IntervalAnalysis, intervalsMatch } from './intervalAnalysis';
...
const CHORD_TEMPLATES: Record<string, number[]> = {
    ...
    'm(add9)': [0, 3, 7, 14],
    'm(add4)': [0, 3, 5, 7],
    'add4': [0, 4, 5, 7],     // Major triad + simple P4
    'add11': [0, 4, 7, 17],   // Major triad + compound P4 (11)
    'maj9': [0, 4, 7, 11, 14],
    'm9': [0, 3, 7, 10, 14],
    '9': [0, 4, 7, 10, 14],
    '7b9': [0, 4, 7, 10, 13],
...
export function getPitchClasses(notes: number[]): number[] {
    return [...new Set(notes.map(getPc))].sort((a, b) => a - b);
}
...
/**
 * Compatibility helper: returns the register-aware, root-relative intervals.
 * Use analyzeIntervalStructure when pitch-class and compound semantics are
 * both needed by the caller.
 */
function getIntervals(rootMidi: number, notes: number[]): number[] {
    return analyzeIntervals(rootMidi, notes).absoluteIntervals;
}

export function analyzeIntervalStructure(rootMidi: number, notes: number[]): IntervalAnalysis {
    return analyzeIntervals(rootMidi, notes);
}

/**
 * Exact semantic match: a 2nd and a 9th share a pitch class but not an
 * interval role.  This deliberately rejects [0, 2, 4, 7] for add9.
 */
function matchesTemplate(source: number[], template: number[]): boolean {
    return intervalsMatch(source, template);
}
...
function refineQuality(quality: string, intervals: number[]): { quality: string, alterations: string[] } {
    ...
    const has = (semitones: number) => hasInterval(intervals, semitones);
...
function detectStructure(notes: number[], context: { useFlats: boolean }): ChordResult | null {
    ...
    for (const rootMidi of sorted) {
        ...
        const intervalAnalysis = analyzeIntervals(rootMidi, sorted);
        ...
        for (const [type, template] of Object.entries(CHORD_TEMPLATES)) {
            ...
            if (matchesTemplate(intervals, templateNo5)) {
                ...
                return {
                    ...
                    quality: type,
                    bass: bassName,
                    name: isSlash ? `${rootName}${type}(no5)/${bassName}` : `${rootName}${type}(no5)`,
                    intervals,
                    intervalAnalysis,
                    omissions: ['omit5'],
        ...
        for (const [type, template] of Object.entries(CHORD_TEMPLATES)) {
            ...
            if (matchesTemplate(intervals, templateShell)) {
                ...
                return {
                    ...
                    quality: type,
                    bass: bassName,
                    name: isSlash ? `${rootName}${type}(no3,no5)/${bassName}` : `${rootName}${type}(no3,no5)`,
                    intervals,
                    intervalAnalysis,
    ...
    const intervalAnalysis = analyzeIntervals(bassMidi, sorted);
    const intervals = intervalAnalysis.absoluteIntervals;

    const has11 = hasInterval(intervals, 5, 'compound');
    const hasb7 = hasInterval(intervals, 10);
    const hasm3 = hasInterval(intervals, 3);
...
function generateSymmetricalAlternatives(result: ChordResult, notes: number[], context: { useFlats: boolean }): ChordResult[] {
    ...
    distinctNotes.forEach(pc => {
        ...
        alternatives.push({
            ...
            intervalAnalysis: result.intervalAnalysis,
            omissions: [],
...
function detectPolychord(notes: number[], context: { useFlats: boolean }): ChordResult | null {
    ...
    if (upperResult && upperResult.confidence >= 0.9) {
        return {
            ...
            intervalAnalysis: analyzeIntervals(bassMidi, sorted),
            omissions: [],
