import { analyzeIntervals, hasCompoundInterval, hasInterval, IntervalAnalysis, intervalsMatch as intervalStructuresMatch } from './intervalAnalysis';
...
export interface ChordDetectionResult {
    ...
    /** Root-relative pitch-class and register-aware interval evidence. */
...
const CHORD_PATTERNS: { [key: string]: number[] } = {
    ...
    // Add chords (4-note)
    'add4': [0, 4, 5, 7],               // Major add 4 (simple P4)
    'add9': [0, 4, 7, 14],              // Major add 9 (compound M2)
    'add6': [0, 4, 7, 9],               // Major add 6
    'm add6': [0, 3, 7, 9],             // Minor add 6
    'add#9': [0, 4, 7, 15],             // Major add #9 (spanning octave)
    'add11': [0, 4, 7, 17],             // Major add 11 (compound P4)
    'm add4': [0, 3, 5, 7],             // Minor add 4 (simple P4)
    'm add11': [0, 3, 7, 17],           // Minor add 11 (compound P4)
...
export function getPitchClasses(notes: NormalizedNote[]): number[] {
    return [...new Set(notes.map(note => note.pitchClass))].sort((a, b) => a - b);
}
...
export function analyzeIntervalStructure(notes: NormalizedNote[], explicitRoot?: NormalizedNote): IntervalAnalysis {
    if (notes.length === 0) {
        return { pitchClasses: [], absoluteIntervals: [], simpleIntervals: [], compoundIntervals: [] };
    }

    const rootNote = explicitRoot || notes[0];
    return analyzeIntervals(rootNote.midi, notes.map(note => note.midi));
}

function getIntervals(notes: NormalizedNote[], explicitRoot?: NormalizedNote): number[] {
    return analyzeIntervalStructure(notes, explicitRoot).absoluteIntervals;
}
...
function analyzeVoicing(notes: NormalizedNote[], options: ChordAnalysisOptions): ChordDetectionResult | null {
    const intervalAnalysis = analyzeIntervalStructure(notes);
    const intervals = intervalAnalysis.absoluteIntervals;
...
function intervalsMatch(intervals: number[], pattern: number[]): boolean {
    // Do not normalize through % 12 here. That collapses 2nd/9th,
    // 4th/11th, and 6th/13th into the same label.
    return intervalStructuresMatch(intervals, pattern);
}
...
function detectDominantFeatures(intervals: number[], notes: NormalizedNote[], options: ChordAnalysisOptions = {}): ChordDetectionResult | null {
    ...
    const hasMajor3 = hasInterval(intervals, 4);
    const hasMinor3 = hasInterval(intervals, 3);
    const has7 = hasInterval(intervals, 10); // b7
    const hasMaj7 = hasInterval(intervals, 11);
    ...
    const hasSimpleOrCompound = (targetSemitones: number) => hasInterval(intervals, targetSemitones);
    const hasCompound = (targetSemitones: number) => hasCompoundInterval(intervals, targetSemitones);

    const has5 = hasSimpleOrCompound(7);
    const hasb5 = hasSimpleOrCompound(6); // tritone
    const hasSharp5 = hasSimpleOrCompound(8); // minor 6th

    const hasb9 = hasCompound(1);
    const has9 = hasCompound(2);
    const hasSharp9 = hasCompound(3);

    const has11 = hasCompound(5);
    const hasb13 = hasCompound(8); // minor 13th
    const has13 = hasCompound(9);
    ...
    return {
        ...
        intervalAnalysis: analyzeIntervalStructure(notes)
    };
}
...
function detectFunctionalEnharmonics(intervals: number[], notes: NormalizedNote[]): ChordDetectionResult[] {
    ...
    const has7 = hasInterval(intervals, 10) || hasInterval(intervals, 11);
    const has5 = hasInterval(intervals, 7);
    const hasAb = hasInterval(intervals, 8); // Could be #5 or b13
    const hasMajor3 = hasInterval(intervals, 4);
    ...
    const hasEb = hasInterval(intervals, 3); // Could be b3 or #9
    ...
    for (const noteCombo of noteCombinations) {
        ...
        if (options.whole_detect) {
            ...
            for (const candidateRoot of sortedNotes) {
                ...
                const rotatingAnalysis = analyzeIntervalStructure(sortedNotes, candidateRoot);
                ...
                if (matchType !== null) {
                    ...
                    results.push({
                        ...
                        intervalAnalysis: rotatingAnalysis
                    });
                }
            }
        }
