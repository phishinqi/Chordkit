...
function omissionIntervalsMatch(actual: readonly number[], expected: readonly number[]): boolean {
  return intervalsMatch(actual.map(normalizePitchClass), expected.map(normalizePitchClass));
}
...
export function detectOmissions(intervals: readonly number[], templates: readonly ChordTemplate[]): OmissionMatch[] {
  ...
  for (const template of templates) {
    ...
    if (withoutThird.length < template.intervals.length && omissionIntervalsMatch(intervals, withoutThird)) matches.push({ template, omissions: ['omit3'] });
    const shell = withoutThird.filter((value) => value % 12 !== 7);
    if (shell.length < withoutThird.length && omissionIntervalsMatch(intervals, shell)) matches.push({ template, omissions: ['omit3', 'omit5'] });
  }
  return matches;
}
