...
function HarmonySummary({ locale, progression, renderer }: { locale: Locale; progression: ReturnType<typeof harmony.analyzeProgression>; renderer: harmony.RomanRenderer }) {
  const tx = t(locale);
  const contextLabel = progression.globalContext?.label ?? 'Unknown context';
  return <div className="harmony-grid"><section className="card"><h3>{tx.key}</h3><strong className="key-label">{contextLabel}</strong><ol className="key-candidates">{progression.keyCandidates.map((candidate) => <li key={candidate.context.label}><span>{candidate.context.label}</span><b>{candidate.confidence}</b><small>{candidate.evidence.at(-1)}</small></li>)}</ol></section><section className="card"><h3>{tx.roman}</h3><div className="roman-events">{progression.events.map((event) => <article key={event.id}><small>{event.localContext?.label ?? 'Unknown context'}{event.modulation ? ' · modulation' : ''}</small><strong>{event.analysis.primary?.renderings[renderer] ?? '?'}</strong><span>{event.analysis.primary?.function ?? 'unknown'}</span></article>)}</div></section></div>;
}
...
function TimelineCards({ locale, timeline, renderer, selectedSegment, onSelect }: { locale: Locale; timeline: harmony.HarmonicTimeline; renderer: harmony.RomanRenderer; selectedSegment: number; onSelect: (index: number) => void }) {
  const tx = t(locale);
  const contextLabel = timeline.globalContext?.label ?? 'Unknown context';
  return <section className="card timeline-card"><div className="card-head"><span>{tx.harmonyTimeline}</span><small>{contextLabel}</small></div><div className="timeline-segments">{timeline.segments.map((segment) => <button className={selectedSegment === segment.index ? 'selected' : ''} key={segment.index} onClick={() => onSelect(segment.index)}><small>{segment.timeline.startTick}–{segment.timeline.endTick}</small><strong>{segment.harmony.primary?.renderings[renderer] ?? '?'}</strong><span>{segment.harmony.context?.label ?? contextLabel}{segment.harmony.primary?.function ? ` · ${segment.harmony.primary.function}` : ''}</span></button>)}</div></section>;
}
