...
import type { ScaleCandidate } from '@chordkit/scale';
...
export default function App() {
  ...
  const [scalePreview, setScalePreview] = useState<ScaleCandidate | null>(null);
  ...
  useEffect(() => { const onKey = (event: KeyboardEvent) => { const keys: Record<string, string> = { a:'C3', w:'C#3', s:'D3', e:'D#3', d:'E3', f:'F3', t:'F#3', g:'G3', y:'G#3', h:'A3', u:'A#3', j:'B3', k:'C4' }; if (!keys[event.key] || (['INPUT', 'TEXTAREA', 'SELECT', 'BUTTON'].includes((event.target as HTMLElement)?.tagName) || (event.target as HTMLElement)?.isContentEditable)) return; event.preventDefault(); activateNote(keys[event.key]!); }; window.addEventListener('keydown', onKey); return () => window.removeEventListener('keydown', onKey); });
  ...
  const activateNote = (note: string, midi = core.parseNote(note).midi) => {
    toggleNote(note);
    if (sound) void playNotes([midi]);
    const output = midiAccessRef.current?.outputs.get(midiOutputId);
  ...
  const sendCurrentChordToOutput = () => {
    const output = midiAccessRef.current?.outputs.get(midiOutputId);
    if (output) sendMidiNotes(output, midiFromNotes(workspace.notes));
  };
  const connectMidi = async () => {
    ...
    try {
      ...
      const outputs = listMidiOutputs(access);
      setMidiOutputs(outputs);
      setMidiOutputId((current) => current || outputs[0]?.id || '');
      let sequence = 0;
      access.inputs.forEach((input) => {
        input.onmidimessage = (event) => {
          const data = event.data;
          if (!data) return;
          const [status, note, velocity = 0] = Array.from(data);
          const command = status! >> 4;
          if (command !== 8 && command !== 9) return;
          const now = Math.round(performance.now());
          const item = command === 9 && velocity > 0 ? { type:'noteOn', tick:now, track:0, channel:status! & 15, midi:note!, velocity, sequence:sequence++ } : { type:'noteOff', tick:now, track:0, channel:status! & 15, midi:note!, releaseVelocity:velocity, sequence:sequence++ };
          setLiveEvents((current) => [...current.slice(-63), item]);
          if (sound && command === 9 && velocity > 0) void playNotes([note!]);
        };
      });
      setWebMidi(`${tx.webMidiReady} · ${outputs.length} output(s)`);
    } catch (reason) { setWebMidi(reason instanceof Error ? reason.message : String(reason)); }
  };
  ...
  return <div className="app-shell">
    <header className="masthead"><a className="brand" href="#top" onClick={() => setTab('home')}><span className="brand-mark">C</span><span>Chordkit</span></a><span className="beta">{tx.beta}</span><div className="header-actions"><button onClick={() => setLocale(locale === 'zh' ? 'en' : 'zh')}><Languages size={16}/>{locale === 'zh' ? 'EN' : '中文'}</button><button className={sound ? 'on' : ''} onClick={() => { setSound(!sound); if (!sound) void playNotes(midiFromNotes(workspace.notes)); }}><AudioLines size={16}/>{sound ? tx.soundOn : tx.soundOff}</button><a href="https://github.com/phishinqi/Chordkit" target="_blank"><Github size={16}/>GitHub</a></div></header>
...
export function ResultCard({ result, locale, error, onAddToHarmony }: { result: ReturnType<typeof core.analyzeChord> | null; locale: Locale; error: string; onAddToHarmony?: (candidate: core.ChordCandidate) => void }) {
  const tx = t(locale);
  if (!result) return <div className="card"><p className="error">{error}</p></div>;
  return <div className="card result-card"><span className="index">PRIMARY CANDIDATE</span><h2>{result.primary?.name ?? 'No match'}</h2><p>{result.primary?.quality} · score {result.primary?.score}</p>{result.primary && onAddToHarmony && <button className="add-harmony" onClick={() => onAddToHarmony(result.primary!)}>{tx.addToHarmony}</button>}<div className="result-columns"><div><h3>{tx.candidates}</h3>{result.candidates.map((candidate, index) => <div className="candidate-row" key={`${candidate.name}-${index}`}><p><b>{candidate.name}</b><span>{candidate.score}</span></p>{candidate.omissions.length > 0 && <div className="omission-tags">{candidate.omissions.map((omission) => <small key={omission}>{omissionLabel(omission, locale)}</small>)}</div>}{onAddToHarmony && <button className="candidate-add" onClick={() => onAddToHarmony(candidate)}>{tx.addToHarmony}</button>}</div>)}</div><div><h3>{tx.relations}</h3>{result.relations.length ? result.relations.map((relation) => <p key={`${relation.type}-${relation.target}`}>{relation.target}</p>) : <p>—</p>}</div></div><JsonPanel locale={locale} value={result}/></div>;
}
